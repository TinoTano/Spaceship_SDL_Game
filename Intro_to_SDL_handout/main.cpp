...
struct globals
{
	...
	int last_shot;
	bool fire, up, down, left, right, misile;
	KEY_STATE* keyboard;
	// TODO 4:
...
void Start()
{
	...
	g.laser_fx = Mix_LoadWAV("laser.wav");
	...
	g.background_scroll = SCROLL_SPEED;
...
bool CheckInput()
{
	...
	g.misile = g.keyboard[SDL_SCANCODE_C] == KEY_REPEAT;
...
void MoveStuff()
{
	...
	if (g.left && !g.right && g.ship_x > 0){
		g.ship_x -= 4;
	}
	else if (g.right && !g.left && g.ship_x < SCREEN_WIDTH - 64){
		g.ship_x += 4;
	}
	else if (g.up && !g.down && g.ship_y > 0){
		g.ship_y -= 4;
	}
	else if (g.down && !g.up && g.ship_y < SCREEN_HEIGHT - 64){
		g.ship_y += 4;
	}
	...
	if(g.fire)
	{
		...
		g.last_shot++;
		Mix_PlayChannel(-1, g.laser_fx, 0);
	}
...
void Draw()
{
	...
	if (g.background_scroll >= g.background_width){
		...
		if (!g.misile){
			...
			g.background_scroll += SCROLL_TURBO;
		}
	}
